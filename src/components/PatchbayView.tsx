// PatchbayFaceProps interface must be at the very top, before any code
interface PatchbayFaceProps {
  segments: BaySegment[];
  ...
  segmentButtonProps?: React.ButtonHTMLAttributes<HTMLButtonElement>;
}
  ...
  return (
    <div className="flex items-center gap-2">
      <button type="button" onClick={onSelect} className="flex min-w-0 flex-1 items-center gap-2 text-left">
        <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-[10px] font-medium ${selected ? bayToneClasses[tone].selected : `bg-zinc-950/95 ${bayToneClasses[tone].socket}`}`}>{pointNumber}</span>
      ...
      <button type="button" onClick={onInspect} className="shrink-0 rounded-full border border-zinc-700 px-2 py-1 text-[10px] uppercase tracking-[0.12em] text-zinc-400 hover:text-zinc-200">Inspect</button>
    </div>
  );
}
            ...
            >
              <div className="space-y-3">
                ...
                <PatchbayFace
                  ...
                  segmentButtonProps={row.id === 'row-mic-ties' ? ({ 'data-mic-segment': 'true' } as any) : undefined}
                />
              </div>
            </div>
