...
export default function SonicSignatureStrip({ signature }: Props) {
  ...
  return (
    <div className="border-t border-zinc-800/60 bg-zinc-950/80 px-4 py-2 backdrop-blur">
      <div className="flex items-center gap-6">
        ...
        <div className="min-w-0 flex-1">
          <p className={`truncate text-xs leading-relaxed transition-all duration-500 ${hasStages ? 'text-zinc-300' : 'text-zinc-600'}`}>
            {signature.summary}
          </p>
        </div>
        ...
        {/* Stage count chip */}
        {hasStages && (
          <div className="shrink-0 rounded-full border border-zinc-700/50 bg-zinc-900/60 px-2 py-0.5 text-[10px] text-zinc-400">
      ...
      {signature.stages.length > 1 && (
        <div className="mt-1.5 flex items-center gap-2 overflow-x-auto text-[10px] text-zinc-600">
