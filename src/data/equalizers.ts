...
export const equalizers: Equalizer[] = [
  ...
  // ══════════════════════════════════════════════════════
  {
    ...
    channels: 8,
    topology: 'tilt',
    bands: 'Single tilt control per channel — rotates entire spectral balance around 700Hz center frequency. Clockwise = brighter, counter-clockwise = darker',
    has_transformer: false,
    input_z_ohm: 10000,
    output_z_ohm: 75,
    noise_floor_db: -95,
    rack_units: 1,
    em_zone: 'C',
    character: 'Subtle and transparent. One knob per channel reshapes the entire tonal balance without any single frequency jumping out. Eight channels cover Aurora DA 1–8 → API ch 1–8 by default.',
    engineering: 'Discrete Class A tilt EQ. 8 channels, each with independent tilt control pivoting around 700Hz, per-channel hard-wire bypass switch, and per-channel polarity reverse toggle. ToneLux discrete op-amps. Extremely low noise floor.',
  ...
  },
  {
    ...
    rack_units: 1,
    em_zone: 'C',
    character: 'Subtle and transparent. One knob per channel reshapes the entire tonal balance without any single frequency jumping out. Eight channels cover Aurora DA 9–16 → API ch 9–16 by default.',
    engineering: 'Discrete Class A tilt EQ. 8 channels, each with independent tilt control pivoting around 700Hz, per-channel hard-wire bypass switch, and per-channel polarity reverse toggle. ToneLux discrete op-amps. Extremely low noise floor.',
    tendencies: ['Shifts a source brighter or darker as a whole instead of drawing attention to individual bands', 'Makes quick tonal course-correction possible without collapsing into over-EQing', 'Often preserves the impression of the original source while nudging balance into place'],
    tradeoffs: ['Single-knob simplicity means you cannot solve low and high issues independently', 'Because the move feels gentle, it can be tempting to use it where a more targeted fix is actually needed', 'The result can be subtle enough that careful bypass comparison is needed to judge whether it helped'],
    workflow_implications: ['Excellent momentum tool when a source just needs to lean warmer or brighter and move on', 'Useful late in a chain for tonal re-centering after compression or other coloring stages', 'Pairs well with faster decision-making because the control set discourages overworking'],
